...
def fetchDataset(dataset='iris'):
    if dataset == 'iris':
        ...
        y = genfromtxt('irisY.txt', delimiter=',', dtype=np.int64) - 1  # FIXED
        pcadim = 2
    elif dataset == 'wine':
        X = genfromtxt('wineX.txt', delimiter=',')
        y = genfromtxt('wineY.txt', delimiter=',', dtype=np.int64) - 1  # FIXED
        pcadim = 0
    elif dataset == 'olivetti':
        X = genfromtxt('olivettifacesX.txt', delimiter=',')
        X = X / 255
        y = genfromtxt('olivettifacesY.txt', delimiter=',', dtype=np.int64)  # FIXED
        pcadim = 20
    elif dataset == 'vowel':
        X = genfromtxt('vowelX.txt', delimiter=',')
        y = genfromtxt('vowelY.txt', delimiter=',', dtype=np.int64)  # FIXED
        pcadim = 0
    else:
...
class DecisionTreeClassifier(object):
    ...
    def trainClassifier(self, Xtr, yTr, W=None):
        ...
        rtn.classifier = tree.DecisionTreeClassifier(max_depth=int(Xtr.shape[1] / 2 + 1))  # FIXED
        if W is None:
